...
ComponentCssPostprocessor.prototype.write = function (readTree, destDir) {
  ...
  return readTree(this.inputTree).then(function(srcDir) {
    ...
    console.log("Writing ember-component-css JS to this file: ", vendorjs);
    fs.appendFileSync(path.join(destDir, 'assets', vendorjs), cssInjectionSource);

    console.log("After writing, assets:", fs.readdirSync(path.join(destDir, 'assets')));
    
    // Only if we generated a pod-styles.css file do we need to append to vendor.css
    if (pod.extension === 'css') {
      var vendorcss = assetFiles.filter(function(i){ return i.match(/vendor.*?\.css/) })[0];
      console.log("Writing ember-component-css CSS to this file: ", vendorcss);
      fs.appendFileSync(path.join(destDir, 'assets', vendorcss), pod.styles);
    }
